...
export async function registerUploadRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Querystring: { sessionId?: string } }>(
    ...
    async (req, reply) => {
      ...
      const kuyruk = await getParseQueue();
      // sessionId is emrinde tasiniyor: worker belgeyi kalici olarak bu
      // oturuma bagliyor ve analiz/yonlendirmeyi atliyor. Boylece ek belge
      // hicbir zaman servis havuzuna dusmuyor.
      await kuyruk.add(
        "parse",
